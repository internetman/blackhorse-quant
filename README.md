...
首页卡片和表格行都支持人工归档。归档记录由 `m2-archive.js` 按股票代码写入当前域名的浏览器 `localStorage`，刷新页面、重新部署和下一次收盘数据更新后仍会保持；首页与 `/m2-table` 共用归档箱并可恢复。人工归档只改变当前浏览器的候选视图，不删除行情、历史 K 线或阶段档案，也不会同步到其它浏览器或其它域名。
